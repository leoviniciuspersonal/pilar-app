...
export default function Today({ user, profile }) {
  ...
  async function loadStats() {
    ...
    for (let i = 0; i < days.length; i++) {
      ...
      if (avg > 0) {
        ...
        if (i === 0 || dateDiff(days[i], days[i - 1]) === 1) streak = cur
      } else { cur = 0 }
    }
  ...
  const firstName = profile?.name?.split(' ')[0] || 'Léo'
  ...
  return (
    <div>
      ...
      <div className="day-score-card card" style={{ marginBottom: '1rem' }}>
        <div>
          <div className="day-score-sub">pontuação do dia</div>
      ...
      <div className="stat-row">
        ...
        <div className="stat-card">
          ...
          <div className="stat-lbl">melhor sequência</div>
        </div>
        <div className="stat-card">
          <div className="stat-num">{stats.total}</div>
          <div className="stat-lbl">dias registrados</div>
        </div>
      </div>
      ...
      <div className="pillars-grid">
        {PILLARS.map(p => {
          ...
          return (
            <div
              key={p.id}
              className={`pillar-card ${done ? 'done' : ''}`}
